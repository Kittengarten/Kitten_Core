...
// HandshakeFn returns the handshakeFn.
func (c *Conn) HandshakeFn() *func(context.Context) error {
...
func Use(conn *tls.Conn) *Conn {
	return (*Conn)(unsafe.Pointer(conn))
}

// HandshakeWithFirstFragmentLen do terasu handshake in this TLS conn
func (conn *Conn) HandshakeWithFirstFragmentLen(firstFragmentLen uint8) error {
	expose := conn
	fnbak := expose.handshakeFn
	expose.handshakeFn = conn.clientHandshakeWithFirstFragmentLen(firstFragmentLen)
	defer func() { expose.handshakeFn = fnbak }()
	return conn.Handshake()
}

// HandshakeContextWithFirstFragmentLen do terasu handshake with ctx in this TLS conn
func (conn *Conn) HandshakeContextWithFirstFragmentLen(ctx context.Context, firstFragmentLen uint8) error {
	expose := conn
	fnbak := expose.handshakeFn
	expose.handshakeFn = conn.clientHandshakeWithFirstFragmentLen(firstFragmentLen)
	defer func() { expose.handshakeFn = fnbak }()
