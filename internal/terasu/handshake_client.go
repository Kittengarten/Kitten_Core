...
import (
	...
	"github.com/fumiama/terasu/internal/hpke"
	"github.com/fumiama/terasu/internal/mlkem768"
)
...
func (cout *Conn) clientHandshakeWithFirstFragmentLen(firstFragmentLen uint8) func(context.Context) error {
	return func(ctx context.Context) (err error) {
		...
		hello, keyShareKeys, ech, err := (*Conn)(unsafe.Pointer(c)).makeClientHello()
		if err != nil {
			return err
		}
		c.serverName = hello.serverName
		...
		if _, err := c.writeHandshakeRecordWithFirstFragmentLen(hello, nil, firstFragmentLen); err != nil {
			return err
		}
...
// written to it.
func (c *Conn) writeHandshakeRecordWithFirstFragmentLen(msg handshakeMessage, transcript transcriptHash, firstFragmentLen uint8) (int, error) {
	c.out.Lock()
	defer c.out.Unlock()
	...
	return c.writeRecordLockedWithFirstFragmentLen(recordTypeHandshake, firstFragmentLen, data)
}

// writeRecordLocked writes a TLS record with the given type and payload to the
// connection and updates the record layer state.
func (c *Conn) writeRecordLockedWithFirstFragmentLen(typ recordType, firstFragmentLen uint8, data []byte) (int, error) {
	if c.quic != nil {
