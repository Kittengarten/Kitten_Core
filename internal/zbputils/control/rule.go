...
// managers 每个插件对应的管理
var managers = ctrl.NewManager[*zero.Ctx](dbfile)
...
func init() {
	...
	zero.OnCommandGroup([]string{
		...
		if !ok {
			ctx.Send(message.Text("没有找到指定服务!"))
			return
		}
		grp := ctx.Event.GroupID
		if grp == 0 {
		...
		if strings.Contains(model.Command, "启用") || strings.Contains(model.Command, "enable") {
			...
			if service.Options.OnDisable != nil {
				...
				ctx.Send(message.Text("已禁用服务: " + model.Args))
			}
		}
	})
	...
	zero.OnCommandGroup([]string{
		...
		if condition {
			...
			ctx.Send(message.Text("此处启用所有插件成功"))
		} else {
			managers.ForEach(func(key string, manager *ctrl.Control[*zero.Ctx]) bool {
				manager.Disable(grp)
				return true
			})
			ctx.Send(message.Text("此处禁用所有插件成功"))
		}
	})
	...
	zero.OnCommandGroup([]string{
		...
		if strings.Contains(model.Command, "启用") || strings.Contains(model.Command, "enable") {
			service.Enable(0)
			ctx.Send(message.Text("已全局启用服务: " + model.Args))
		} else {
			service.Disable(0)
			ctx.Send(message.Text("已全局禁用服务: " + model.Args))
		}
	})
	...
	zero.OnCommandGroup([]string{"还原", "reset"}, zero.UserOrGrpAdmin, zero.OnlyToMe).SetBlock(true).SecondPriority().Handle(func(ctx *zero.Ctx) {
		...
		service.Reset(grp)
		ctx.Send(message.Text("已还原服务的默认启用状态: " + model.Args))
	})
	...
	zero.OnCommandGroup([]string{
		...
		ctx.Send(message.Text("已改变全局默认启用状态: " + model.Args))
	})
	...
	zero.OnCommandGroup([]string{"用法", "usage"}, zero.OnlyToMe).SetBlock(true).SecondPriority().
		Handle(func(ctx *zero.Ctx) {
			...
			if id := ctx.Send(message.ImageBytes(data)); id.ID() == 0 {
	...
	zero.OnCommandGroup([]string{"服务列表", "service_list"}, zero.OnlyToMe).SetBlock(true).SecondPriority().
		Handle(func(ctx *zero.Ctx) {
			...
			if len(imgs) > 1 {
				...
				if id := ctx.Send(message.Image("base64://" + binary.BytesToString(b64))); id.ID() == 0 {
					ctx.Send(message.Text("ERROR: 可能被风控了"))
				}
			}
		})
	...
	zero.OnCommand("设置服务列表显示行数", zero.SuperUserPermission, zero.OnlyToMe).SetBlock(true).SecondPriority().Handle(func(ctx *zero.Ctx) {
		...
		if err != nil {
			ctx.Send(message.Text("请输入正确的数字"))
			return
		}
		...
		ctx.Send(message.Text("已设置列表单页显示数为 " + strconv.Itoa(lnperpg)))
	})
}
