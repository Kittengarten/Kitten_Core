...
func unpack(raw string) (nturl, error) {
	...
	b := rb[ntappidlen-1]
	fileid := base64.RawURLEncoding.EncodeToString(rb[:ntappidlen-1])
	if len(fileid) < int(b) {
		return "", ErrInvalidNTRaw
	}
	fileid = fileid[:b]
	rkey := base64.RawURLEncoding.EncodeToString(rb[ntappidlen:])
	b = rb[ntrawlen-1]
	if len(rkey) < int(b) {
		return "", ErrInvalidNTRaw
	}
...
func (nu nturl) pack() (string, error) {
	...
	_, err = base64.RawURLEncoding.AppendDecode(buf[ntappidlen:ntappidlen], binary.StringToBytes(rkey))
	if err != nil {
		return "", err
	}
